...
def get_token_by_ticker(agent, **kwargs):
    ...
    try:
        ...
        token_address = agent.connection_manager.connections[
            "ethereum"
...
@register_action("transfer-custom")
def transfer_custom(agent, **kwargs):
    """Transfer custom tokens"""
    try:
        return agent.connection_manager.connections["ethereum"].transfer_custom(
            **kwargs
        )
    except Exception as e:
        logger.error(f"Failed to transfer custom tokens: {str(e)}")
        return None


@register_action("transfer-prompt")
def transfer_prompt(agent, **kwargs):
    """Transfer prompt"""
    try:
        return agent.connection_manager.connections["ethereum"].transfer_prompt()
    except Exception as e:
        logger.error(f"Failed to transfer prompt: {str(e)}")
        return None
