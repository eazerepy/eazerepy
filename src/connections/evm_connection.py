...
class EVMConnection(BaseConnection):
    def __init__(self, config: Dict[str, Any]):
        ...
        self.aggregator_api = (
            f"https://aggregator-api.kyberswap.com/{self.network}/api/v1"
        )
        ...
        if not self._web3:
            for attempt in range(3):
                try:
                    ...
                    logger.info(
                        f"Connected to {self.network} network with chain ID: {chain_id}"
                    )
                    break
    ...
    def register_actions(self) -> None:
        ...
        self.actions = {
            "get-token-by-ticker": Action(
                ...
                parameters=[
                    ActionParameter(
                        "ticker", True, str, "Token ticker symbol to look up"
                    )
                ],
                description="Get token address by ticker symbol",
            ),
            "custom-get-balance": Action(
                name="custom-get-balance",
                parameters=[
                    ActionParameter(
                        "address", True, str, "Address to check balance for"
                    ),
                    ActionParameter(
                        "token_address",
                        False,
                ...
                description="Get balance for a custom address",
            ),
            "get-balance": Action(
                name="get-balance",
                parameters=[
            ...
            "transfer-custom": Action(
                name="transfer-custom",
                parameters=[
                    ActionParameter("to_address", True, str, "Recipient address"),
                    ActionParameter("amount", True, float, "Amount to transfer"),
                    ActionParameter("private_key", True, str, "Private key"),
                    ActionParameter(
                        "token_address",
                        False,
                        str,
                ...
                description="Send ETH or tokens with a custom private key",
            ),
            "get-address": Action(
                name="get-address",
                parameters=[],
    ...
    def configure(self) -> bool:
        ...
        try:
            ...
            if len(private_key) != 66 or not all(
                c in "0123456789abcdefABCDEF" for c in private_key[2:]
            ):
    ...
    def custom_get_balance(
        self, address: str, token_address: Optional[str] = None
    ) -> float:
        """
        Get balance for a custom address.
        If token_address is None, the native token balance is returned.
        """
        try:
            if token_address is None:
                raw_balance = self._web3.eth.get_balance(address)
            ...
            logger.error(f"Failed to get balance error-message: {str(e)}")
            return False
    ...
    def _prepare_transfer_tx(
        self, to_address: str, amount: float, token_address: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def transfer(
        ...
        try:
            ...
            private_key = os.getenv("EVM_PRIVATE_KEY") or os.getenv("ETH_PRIVATE_KEY")
            logger.info(f"DEBUG transfer: private_key: {private_key}")
            account = self._web3.eth.account.from_key(private_key)
    ...
    def transfer_custom(
        ...
        try:
            logger.info(
                f"DEBUG transfer-custom parameters: {to_address}, {amount}, {private_key}, {token_address}"
            )
            current_balance = self.get_balance(token_address=token_address)
            logger.info(f"DEBUG transfer-custom current_balance: {current_balance}")
            if current_balance < amount:
                raise ValueError(
                    f"Insufficient balance. Required: {amount}, Available: {current_balance}"
                )
            tx = self._prepare_transfer_tx(to_address, amount, token_address)
            account = self._web3.eth.account.from_key(private_key)
            logger.info(f"DEBUG transfer-custom account: {account}")
            signed = account.sign_transaction(tx)
            tx_hash = self._web3.eth.send_raw_transaction(signed.rawTransaction)
            tx_url = self._get_explorer_link(tx_hash.hex())
            return tx_url
    ...
    def _get_swap_route(
        self, token_in: str, token_out: str, amount: float, sender: str
    ) -> Dict:
    ...
    def _build_swap_tx(
        ...
        try:
            ...
            tx = {
                ...
                "data": data["data"]["data"],
                "value": (
                    self._web3.to_wei(amount, "ether")
                    if token_in.lower() == self.NATIVE_TOKEN.lower()
                    else 0
                ),
                "nonce": self._web3.eth.get_transaction_count(account.address),
    ...
    def swap(
        self, token_in: str, token_out: str, amount: float, slippage: float = 0.5
    ) -> str:
        ...
        try:
            ...
            current_balance = self.get_balance(
                token_address=(
                    None if token_in.lower() == self.NATIVE_TOKEN.lower() else token_in
                )
            )
            if current_balance < amount:
                raise ValueError(
